...
| M2 Config module | Done (PR #81) | Typed `configuration()` factory + class-validator `validation.schema.ts` covering the env contract of Sure's `.env.example`; `ConfigModule.forRoot({ isGlobal, load, validate })`; `SECRET_KEY_BASE` required outside tests; fail-fast with readable errors; `apps/api/.env.example` as runnable baseline; `main.ts` reads typed port via `ConfigService` |
| M3 Prisma + PostgreSQL schema | Done (PR #82) | 136 models + 25 enums; full faithful `schema.rb` port; decisions D1–D11 applied; `vector(1024)` + pgvector extension (D3/D4); `GENERATED ALWAYS AS (...) STORED` columns for `accounts.classification` + `balances.*Balance` (D2); CHECK/FK constraints hand-applied in init migration; 2 migrations (`init` + `add_auth_refresh_sessions`); verified 0 drift vs schema |
| M4 Redis + BullMQ | Done (PR #82) | `@nestjs/bullmq` + `bullmq` + `ioredis`; queues registered with Sure priority ordering (`scheduled > high_priority > medium_priority > low_priority > default`); trivial `ping` processor proves Redis connectivity |
| M5 Cross-cutting concerns | Done (PR #82) | Global `HttpExceptionFilter` envelope; global `ValidationPipe` (whitelist+transform); `nestjs-pino` structured logging; `@nestjs/swagger`/OpenAPI wired in `main.ts` |
| M6 Auth module | Done | Email/password auth + JWT, TOTP MFA, WebAuthn/passkey, AuthGuard/RolesGuard/PoliciesGuard, fully covered with 43 passing unit tests |
| M7 Families & Users (tenancy) | Done | Scaffolded `FamiliesModule` & `UsersModule` via Nest CLI, implemented family settings/invitations/members, created `@CurrentFamily` tenancy decorator, and added cross-tenant data isolation E2E test |
| M8 Accounts | Done | Scaffolded `AccountsModule` via Nest CLI, modeled account-type discriminators, created shared multi-currency `Money` value-object utility, added balance history & holdings endpoints, and verified family scoping via E2E test |
| M9 Transactions | Done | Scaffolded `TransactionsModule` via Nest CLI, implemented CRUD/filtering/splits/transfers/bulk operations, and verified tenant isolation via E2E test |
| M10 Categories, Merchants, Rules | Done | Scaffolded `CategoriesModule`, `MerchantsModule`, and `RulesModule` via Nest CLI, enforced 1-level category depth, implemented merchant deduplication & rules evaluation engine, covered with unit & E2E tests |
| M11 Budgets & Goals | Done | Scaffolded `BudgetsModule` & `GoalsModule` via Nest CLI, implemented category allocations, family vs personal budgets, goal pledges, envelope progress calculations, DTO/Entity file separation across all modules, and covered with unit & E2E tests |
| M12 Recurring Transactions | Done | Scaffolded `RecurringTransactionsController` & `RecurringTransactionsService`, implemented recurring schedule creation, recurrence rules, occurrence status updates (scheduled/paid/skipped/missed), and covered with unit tests |
